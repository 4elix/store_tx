...
from .models import Categories, Products, Brand, Comments, FavoriteProduct, ViewsProduct, RatingProduct
...
#             # тогда мы ее сохраняем
#             request.session.sava()
#
#         # получаем сессию
#         session_key = request.session.session_key
#         # проверяем есть лм просмотр
#         status_view = ViewsProduct.objects.filter(product=product, user_session=session_key).exists()
#         # если нету просмотра и сессия не None
...
# класс DetailView -> позволяет создать класс для детальной страницы
class ShowDetailView(DetailView):
    # указываем модель для получения данных
    model = Products
    # указываем по какому ключу можно это получить
    context_object_name = 'product'
    # указываем по какому значению найти данный
    slug_url_kwarg = 'slug_path'
    # pk_url_kwarg = -> если переход идет через id продукта
...
# ListView -> класс для формирования QuerySet списка
...
def favorite_logic(request, pk_product):
    # get_object_or_404 -> делает запрос : YouModel.objects.get()
    # если продукта нету, с указанной id, мы получим страницу 404
    product = get_object_or_404(Products, pk=pk_product)
    user = request.user
    # exists -> отдаст True если есть фаворит в базе, отдаст False если его нету
    # работает с filter
    ...
    if status_favorite is True:
        ...
        # save -> сохранит объект
        favorite.save()

    # с помощью данной конструкции, переносит на страницу где была активирована функция
...
# TODO: 6 блок
# TODO: Реализация оплаты part 1 :
# TODO: Реализация оплаты part 2 :
# TODO: Реализация оплаты part 3 :
