...
class CartForAuthenticated:
    ...
    # метод для вывода данных из корзины
    def get_cart_info(self):
        customer, created = Customer.objects.get_or_create(user=self.user)  # создаст или отдаст покупателя
        order, created = Order.objects.get_or_create(customer=customer)  # создаст или отдаст заказ
        order_product = order.orderproduct_set.all()  # получаем заказанные продукты
